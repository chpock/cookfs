    ...
    Tcl_Size origSize)
{
    ...
    if (!resultSize) {
        CookfsLog(printf("ERROR: BrotliEncoderMaxCompressedSize failed"));
        return NULL;
    }
    ...
    CookfsLog(printf("call BrotliEncoderCompress() level %d ...", level));
    ...
    Tcl_Size sizeUncompressed, Tcl_Obj **err)
{
    ...
    CookfsLog(printf("call BrotliDecoderDecompress() ..."));
